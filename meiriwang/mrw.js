...
async function init(startPage, endPage) {
    ...
    http.createServer(function(request, response) {
        ...
        response.write("<!DOCTYPE html>" + "<html>" + "<head>" + '<meta charset="utf-8" />' + "<title>预览图集</title>" + "</head>" + renderCollectHtml() + "</html>");
        response.end();
    }).listen(8888);
...
async function getAndSaveImg(page) {
    ...
    for (let i = 0; i < lis.length; i++) {
        ...
        for (let j = 1; j <= num; j++) {
            let link = config.href.replace("https://www.meituri.com/a/", "https://ii.hywly.com/a/1/");
